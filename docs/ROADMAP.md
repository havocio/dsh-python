...
- [~] **`shell-env`**（`ctx.shellEnv` 注册表：`services/shell_env.py` 已写，含 `DSH_*` 内置变量 + session-persistence contributor；待接线 `tool_bash`/`tool_bash_persistent` 的 `collect(execution)` 调用 + 注册默认 profile + 补单测）
