...
class FileSystem(Service):
    ...
    def read_text(
        ...
        行长超限截断，字节上限用尽即停（``truncated=True``）。
        """
    ...
    def write_text(self, path: str, content: str, actor: Any = None) -> dict:
        """原子写文本（临时文件 + rename；写前广播 ``fs/write-intent`` 守卫）。

        返回 ``{"path", "bytes"}``。
        """
        absolute = self.resolve(path)
        self.ctx.events.dispatch("fs/write-intent", {"path": absolute}, actor)
        ...
        try:
            ...
            if os.path.exists(tmp):
                os.unlink(tmp)
            raise
        self.ctx.emit("fs/observed", {"path": absolute, "present": True, "version": 1}, actor)
    ...
    def edit_text(self, path: str, old_string: str, new_string: str,
        ...
        - 先读版本（``fs/edit-intent`` 守卫可拒绝/续接），再在同一临界区内
          重读、校验、重写（版本检查 + 字面匹配 + 重写共享一个临界区）。
        ...
        self.ctx.events.dispatch("fs/edit-intent", {"path": absolute}, actor)
