...
> **现状**：第 0/2/4 层全部完成；第 3 层支撑服务 **41 包代码已落地**——含 `code-runtime`/`sandbox`/`lsp`/`acp`/`tmux-context`/`e2b` 六包（按「seam + 本地后端/占位」范式翻译，均通过 `py_compile`，**暂未做运行时验证与单测**）、**`web` 家族**（web seam + 抓取/搜索 provider + `web_search`/`web_fetch` 工具）、**`skill` 家族**（`ctx.skills` 分层注册表 + filesystem/badge provider + `skill` 工具与目录/手势注入，先移植了 `dsh-scope` 子系统）、**`workspace`**（`ctx.workspaceRegistry` 耐久工作区注册表）、**`preset`**（`ctx.agentPresets` 预设注册表：发现/创作/常驻挂载 + `persona` 行，依赖 PyYAML）、**`interaction` 家族**（`ctx.commands` 命令注册表 + `ctx.approval` 批准服务 + `ctx.userQuestions` 提问 seam + `ctx.permissionPresets` 权限预设 + `ask_user_question` 工具）、**`subagent-acp`**（外进程 ACP 子代理后端：`ctx.subagents` backend 分派 + stdio ndjson 客户端 + dispose 阶梯）；web/skill/workspace/preset/interaction/subagent-acp 均端到端冒烟已验证；第 4 层 **`--webui` 浏览器前端**（三栏布局对齐 dsh + `console.*` 方法面 + 命令面板/目标/技能/工作区/jobs/设置面板，示例装配 `examples/webui-demo-profile.py`）；**59 个测试模块全绿**（约 500+ 断言）；框架内核零第三方依赖。
...
`--webui` 通过 `websockets.serve` 的 `process_request` 钩子在**同一端口**伺服
内置单页前端（`dsh_py/webui/index.html`，零依赖、离线可用）：浏览器打开
`http://localhost:8080/` 即得聊天界面（要求 websockets ≥ 13 的
`process_request(connection, request)` 签名），页面自动连 `ws://localhost:8080/ws`。

前端**视觉对齐 dsh 的 Web UI**（三栏 AppFrame：侧栏 280px/折叠 56px | 对话 |
详情 360px，`--dsw-*` 设计 token 提炼自 dsh `ui-theme`：neutral-bluish 色阶 +
deepseek 蓝主色 + 双主题语义），**功能面板消费新增的 `console.*` 方法面**：

| 面板 | 方法面 | 后端 seam |
|---|---|---|
| 对话（流式/turn/工具卡片） | `session/prompt` + `session.event` | agent/session |
| 命令面板（Ctrl+K / 输入 `/`） | `console/commands/list` · `execute` | `ctx.commands` |
| 目标 | `console/goals/get` | `ctx.goals` |
| 技能 | `console/skills/list` · `get` | `ctx.skills` |
| 工作区（侧栏列表 + 面板） | `console/workspaces/list` | `ctx.workspaceRegistry` |
| 后台任务 | `console/jobs/list` | `ctx.jobs` |
| 设置（服务可用性） | `console/info` | 装配探测 |

各方法**缺服务时返回 `available:false`**，前端不隐藏面板而是显示**装配引导卡**
（提示用 `--patch` 装配对应 seam）——装了什么 profile 就亮什么面板：

```bash
# 全面板点亮示例：commands + goal + skills 全家桶 + jobs + storage/workspace
python -m dsh_py.gateway --port 8080 --mock --webui \
  --patch examples/webui-demo-profile.py
```

前端交互细节：点侧栏业务面板（目标/技能/工作区/任务/设置）自动切到右侧「面板」
tab 并渲染（此前曾把内容渲染进默认隐藏的 tab 导致“点了没反应”，已修）；点「对话」
切回事件流；命令面板 `Ctrl+K` 或输入 `/` 弹出，回车执行并把结果注入对话；
连接中断自动重连（退避 3 次后弹连接浮层）。
...
| **第 3 层 · 支撑服务**    | 41 个功能包（见 8.2 清单）；六包按「seam + 本地后端/占位」翻译、代码已落地、**未经运行时验证与单测**（见 8.3）；`web`/`skill`/`workspace`/`preset`/`interaction`/`subagent-acp` 已冒烟验证                                  | ✅ 41 包（代码落地）                                   |
...
| **webui 功能面板**              | dsh 前端经宿主 RPC 消费 cordis 服务                                                            | dsh_py 新增 **`console.*` 方法面**（info/commands/goals/skills/workspaces/jobs/plan）直连 `ctx.*` seam；缺服务返回 `available:false`，前端显示装配引导卡（`--patch` 点亮）                                                                 |
...
- [ ]  第 4 层收尾：`host`（聚合宿主/桌面 UI）、跨语言 client 完整版
...
- [ ]  完整 Web 应用剩余项：`host`（聚合宿主/桌面 UI）、跨语言 client 完整版、网关鉴权
